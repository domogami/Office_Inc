...
struct Card2: View {
  ...
  var body: some View {
    ZStack {
      // Left Post It
      VStack {
        Text("I’m a little busy, can you come by later?")
      ...
      // Right Post It
      VStack {
        Text("Yeah, I’m not so busy right now")
          .opacity(viewState1.width < -50 ? 1: 0)
          .font(.system(size: 20, weight: .semibold, design: .default))
          .fixedSize(horizontal: false, vertical: true)
          .padding(.horizontal)
      ...
      FlipView(
        ...
        back:
         ( MainCard(
            ...
            text: "I’m Isey - I work for Newspaper Inc. and I’m here on a tip,  can we go outside?",
            customOffset: true
          )
          .frame(width: screen.width * (3/4), height: screen.height / 2)
          .opacity(self.flipped ? 1 : 0)
